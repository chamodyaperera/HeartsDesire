...
$About = $_POST['About'];*/
	...
	$conn = mysqli_connect('localhost', 'root', '', 'hearts desire');
		...
		$Age_u = mysqli_real_escape_string($conn, $_POST['Age_u']);
		$Height_u = mysqli_real_escape_string($conn, $_POST['Height_u']);
		$Weight_u = mysqli_real_escape_string($conn, $_POST['Weight_u']);
		$Salary_u = mysqli_real_escape_string($conn, $_POST['Salary_u']);
		$Education_u = mysqli_real_escape_string($conn, $_POST['Education_u']);
		$Nationality_u = mysqli_real_escape_string($conn, $_POST['Nationality_u']);
		$Religion_u = mysqli_real_escape_string($conn, $_POST['Religion_u']);
		$Skin_u = mysqli_real_escape_string($conn, $_POST['Skin_u']);
		$Eye_color_u = mysqli_real_escape_string($conn, $_POST['Eye_color_u']);
		$Occupation_u = mysqli_real_escape_string($conn, $_POST['Occupation_u']);
 ...
 /* $user_check_query = "SELECT * FROM user_detail WHERE NIC='$NIC' OR Email='$Email' OR Phone='$Phone' LIMIT 1";
  ...
  if (count($errors) == 0) {
   ...
   // $file = addslashes(file_get_contents($_FILES[$Image]["tmp_name"]));
  	...
  	$query = "INSERT INTO user_preference (Preference_id, Personality_id, Age_u, Height_u, Weight_u, Salary_u, Education_u, Nationality_u, Religion_u, Skin_u, Eye_color_u, Occupation_u) VALUES('$b','$b',?,?,?,?,?,?,?,?,?,?)";
	
	$stmt = $conn->prepare($query);
      $stmt->bind_param("iiiissssss",$Age_u,$Height_u,$Weight_u,$Salary_u,$Education_u,$Nationality_u,$Religion_u,$Skin_u,$Eye_color_u,$Occupation_u);
      $stmt->execute();
	
	header('Location: Partner preference.html');
	$stmt->close();
	$conn->close();
